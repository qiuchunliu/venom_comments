...
def parse_with_re(response):
    ...
    for each in commenttextt:
        commenttext.append(each[0].replace('\n', ''))
        # 评论里会有换行，如果不替换掉，在写入csv文件时会把换行也写进去，
...
def parse_with_bs(response):
    ...
    for each in range(temp_num):
        ...
        comment_info.append((temp_time[each].get('title'), temp_comment[each].text.replace('\n', '')))
        # 评论里会有换行，如果不替换掉，在写入csv文件时会把换行也写进去，
    return
...
def parse_with_lxml(response):
    ...
    for each in range(temp_num):
        ...
        comment_info.append((temp_time[each], temp_comment[each].replace('\n', '')))
        # 评论里会有换行，如果不替换掉，在写入csv文件时会把换行也写进去，
    return
